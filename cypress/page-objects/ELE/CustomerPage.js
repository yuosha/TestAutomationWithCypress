...
//Both, finnish-business-ids AND finnish-ssn originate from the source: https://github.com/vkomulai/
...
class CustomerPage extends BasePage {
    ...
    saveBusNameAndRegCode(filename) {
        ...
        cy.get('#m_txtBusinessName').focus().type('BusinessInc' + this.getRandIntInRange(1000, 100000)) 
    ...
    addEmail() {
        ...
        cy.get('#m_NewContact_value').type('cypmail'+ this.getRandIntInRange(1000, 100000)+'@gmail.com') 
    ...
    addAddress() {
        cy.get('#m_btnAddClientAddress').click() 

        cy.get('#m_bldg_txtZipCode').should('be.visible').type(Cypress.env('clntZip'), {force: true}).wait(2500) // ZIP
        cy.get('#m_bldg_txtSettlement').focus().wait(1250)

        cy.get('#m_bldg_txtStreet').focus().type(Cypress.env('clntStreet'), {force: true}).wait(2500) // STREET

        cy.get('#m_bldg_txtBuilding').focus().then( () => {
            const rn = this.getRandIntInRange(500, 9999)
            cy.wait(1000)
            cy.get('#m_bldg_txtBuilding').type(rn, {force: true}).wait(2500) // BUILDING NUMBER
        })
